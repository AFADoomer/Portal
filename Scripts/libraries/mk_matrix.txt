...
Class PortalMatrix4
{
	...
	static PortalMatrix4 create()
	{
		return new("PortalMatrix4").init();
	}

	static PortalMatrix4 identity()
	{
		PortalMatrix4 o = PortalMatrix4.create();
	...
	PortalMatrix4 add( PortalMatrix4 o )
	{
		PortalMatrix4 r = PortalMatrix4.create();
		int i, j;
	...
	PortalMatrix4 mul( PortalMatrix4 o )
	{
		PortalMatrix4 r = PortalMatrix4.create();
		int i, j;
		for ( i=0; i<4; i++ ) for ( j=0; j<4; j++ )
	...
	static PortalMatrix4 rotate( Vector3 axis, double theta )
	{
		PortalMatrix4 r = PortalMatrix4.identity();
		double s, c, oc;
		s = sin(theta);
		c = cos(theta);
		oc = 1.0-c;
	...
	static PortalMatrix4 perspective( double fov, double ar, double znear, double zfar )
	{
		PortalMatrix4 r = PortalMatrix4.create();
	...
	static Vector3, Vector3, Vector3 getaxes( double pitch, double yaw, double roll )
	{
		...
		PortalMatrix4 mRoll = PortalMatrix4.rotate((1,0,0),roll);
		PortalMatrix4 mPitch = PortalMatrix4.rotate((0,1,0),pitch);
		PortalMatrix4 mYaw = PortalMatrix4.rotate((0,0,1),yaw);
		PortalMatrix4 mRot = mRoll.mul(mYaw);
