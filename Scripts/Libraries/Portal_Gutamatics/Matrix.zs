enum Portal_VectorType {
	Portal_Vector_Position,
	Portal_Vector_Direction
}
...
class Portal_Matrix {
	...
	Portal_Matrix init(int columns, int rows) {
		if (columns <= 0 || rows <= 0) {
	...
	static Portal_Matrix create(int columns, int rows) {
		return new("Portal_Matrix").init(columns, rows);
	}

	/// Returns an identity matrix.
	static Portal_Matrix identity(int dimension) {
		Portal_Matrix ret = Portal_Matrix.create(dimension, dimension);
	...
	static Portal_Matrix fromEulerAngles(double yaw, double pitch, double roll) {
		Portal_Matrix rYaw = Portal_Matrix.identity(4);
		double sYaw = sin(yaw);
		double cYaw = cos(yaw);
		rYaw.set(0, 0,  cYaw);
		rYaw.set(1, 0, -sYaw);
		...
		Portal_Matrix rPitch = Portal_Matrix.identity(4);
		...
		Portal_Matrix rRoll = Portal_Matrix.identity(4);
		...
		Portal_Matrix ret = rYaw.multiplyMatrix(rPitch);
		ret = ret.multiplyMatrix(rRoll);
		return ret;
	}

	/// Returns a rotation matrix from an axis and an angle.
	static Portal_Matrix fromAxisAngle(Vector3 axis, double angle) {
		Portal_Matrix ret = Portal_Matrix.identity(4);
		double c = cos(angle);
		double s = sin(angle);
		double x = axis.x;
		double y = axis.y;
	...
	double, double, double rotationToEulerAngles() {
		if (Portal_GlobalMaths.closeEnough(get(0, 2), -1)) {
			double x = 90;
			double y = 0;
			double z = atan2(get(1, 0), get(2, 0));
			return z, x, y;
		}
		else if (Portal_GlobalMaths.closeEnough(get(0, 2), 1)) {
			double x = -90;
			double y = 0;
			double z = atan2(-get(1, 0), -get(2, 0));
			return z, x, y;
	...
	static Portal_Matrix createTRSEuler(Vector3 translate, double yaw, double pitch, double roll, Vector3 scale) {
		Portal_Matrix translateMat = Portal_Matrix.identity(4);
		translateMat.set(3, 0, translate.x);
		translateMat.set(3, 1, translate.y);
		translateMat.set(3, 2, translate.z);
		
		Portal_Matrix rotateMat = Portal_Matrix.fromEulerAngles(yaw, pitch, roll);
		
		Portal_Matrix scaleMat = Portal_Matrix.identity(4);
		scaleMat.set(0, 0, scale.x);
		scaleMat.set(1, 1, scale.y);
		scaleMat.set(2, 2, scale.z);
		
		Portal_Matrix ret = translateMat.multiplyMatrix(rotateMat);
		ret = ret.multiplyMatrix(scaleMat);
		return ret;
	}
	
	static Portal_Matrix createTRSAxisAngle(Vector3 translate, Vector3 axis, double angle, Vector3 scale) {
		Portal_Matrix translateMat = Portal_Matrix.identity(4);
		translateMat.set(3, 0, translate.x);
		translateMat.set(3, 1, translate.y);
		translateMat.set(3, 2, translate.z);
		
		Portal_Matrix rotateMat = Portal_Matrix.fromAxisAngle(axis, angle);
	...
	static Portal_Matrix view(Vector3 camPos, double yaw, double pitch, double roll) {
		...
		// rotations
		Portal_Matrix rotR = Portal_Matrix.fromAxisAngle((0, 0, 1), roll);
		Portal_Matrix rotP = Portal_Matrix.fromAxisAngle((1, 0, 0), adjustedPitch);
		Portal_Matrix rotY = Portal_Matrix.fromAxisAngle((0, -1, 0), adjustedYaw);
		// pixel ratio scaling
		Portal_Matrix scale = Portal_Matrix.identity(4);
		scale.set(1, 1, pixelRatio);
		// swapping y and z
		Portal_Matrix swapYZ = Portal_Matrix.create(4, 4);
		swapYZ.set(0, 0, 1);
		swapYZ.set(2, 1, 1);
		swapYZ.set(1, 2, -1);
		swapYZ.set(3, 3, 1);
		// translation
		Portal_Matrix translate = Portal_Matrix.identity(4);
		translate.set(3, 0, -camPos.x);
		translate.set(3, 1, -camPos.y);
		translate.set(3, 2, -camPos.z);

		// concatenate them all to get a final matrix
		Portal_Matrix ret = rotR.multiplyMatrix(rotP);
	...
	static Portal_Matrix perspective(double fovy, double aspect, double zNear, double zFar) {
		Portal_Matrix ret = Portal_Matrix.create(4, 4);
	...
	static Portal_Matrix worldToClip(Vector3 viewPos, double yaw, double pitch, double roll, double FOV) {
		double aspect = Screen.getAspectRatio();
		double fovy = Portal_GlobalMaths.fovHToY(FOV);
		Portal_Matrix view = Portal_Matrix.view(viewPos, yaw, pitch, roll);
		// 5 & 65535 are what are used internally, so they're used here for consistency
		Portal_Matrix perp = Portal_Matrix.perspective(fovy, aspect, 5, 65535);
		Portal_Matrix worldToClip = perp.multiplyMatrix(view);
		return worldToClip;
	}
	...
	Portal_Matrix addMatrix(Portal_Matrix other) const {
		if (rows != other.rows || columns != other.columns) {
			throwAbortException("Error: <%p>.addMatrix(<%p>) - Matrices need to be equal size", self, other);
		}
		Portal_Matrix ret = Portal_Matrix.create(columns, rows);
	...
	Portal_Matrix multiplyScalar(double scalar) const {
		Portal_Matrix ret = Portal_Matrix.create(rows, columns);
		for (int row = 0; row < rows; row++) {
			for (int col = 0; col < columns; col++) {
	...
	Portal_Matrix multiplyMatrix(Portal_Matrix other) const {
		if (columns != other.rows) {
			throwAbortException("Error: <%p>.multiplyMatrix(<%p>) - Matrix A columns needs to equal Matrix B rows", self, other);
		}
		Portal_Matrix ret = Portal_Matrix.create(other.columns, rows);
	...
	Portal_Matrix multiplyVector2(Vector2 vec, Portal_VectorType type = Portal_Vector_Position) const {
		Portal_Matrix vec2Matrix = Portal_Matrix.create(1, 3);
		vec2Matrix.set(0, 0, vec.x);
		vec2Matrix.set(0, 1, vec.y);
		if (type == Portal_Vector_Position)       vec2Matrix.set(0, 2, 1);
		else if (type == Portal_Vector_Direction) vec2Matrix.set(0, 2, 0);
	...
	Portal_Matrix multiplyVector3(Vector3 vec, Portal_VectorType type = Portal_Vector_Position) const {
		Portal_Matrix vec3Matrix = Portal_Matrix.create(1, 4);
		vec3Matrix.set(0, 0, vec.x);
		vec3Matrix.set(0, 1, vec.y);
		vec3Matrix.set(0, 2, vec.z);
		if (type == Portal_Vector_Position)       vec3Matrix.set(0, 3, 1);
		else if (type == Portal_Vector_Direction) vec3Matrix.set(0, 3, 0);
	...
	// AFADoomer - Modified from:
	//  http://vr.cs.uiuc.edu/node81.html
	//  https://github.com/VSES/SourceEngine2007/blob/43a5c90a5ada1e69ca044595383be67f40b33c61/src_main/mathlib/vmatrix.cpp#L377
	static Portal_Matrix matrixInverseTR(Portal_Matrix src)
	{
		Portal_Matrix dst = Portal_Matrix.Identity(4);

		Vector3 vTrans, vNewTrans;

		// Transpose the upper 3x3.
		dst.set(0, 0, src.get(0, 0));
		dst.set(1, 0, src.get(0, 1));
		dst.set(2, 0, src.get(0, 2));

		dst.set(0, 1, src.get(1, 0));
		dst.set(1, 1, src.get(1, 1));
		dst.set(2, 1, src.get(1, 2));

		dst.set(0, 2, src.get(2, 0));
		dst.set(1, 2, src.get(2, 1));
		dst.set(2, 2, src.get(2, 2));

		// Transform the translation.
		vTrans = (-src.get(3, 0), -src.get(3, 1), -src.get(3, 2));
		vNewTrans = dst.multiplyVector3(vTrans).asVector3();
		dst.set(3, 0, vNewTrans.x);
		dst.set(3, 1, vNewTrans.y);
		dst.set(3, 2, vNewTrans.z);
